    ...
    <div className="rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50 px-4 py-2 text-sm">
      {hasFilters && (
        <div className="text-slate-500 dark:text-slate-400 text-xs mb-1">
          Filtered by:{" "}
          {[filters.region && `Region: ${filters.region}`, filters.district && `District: ${filters.district}`, filters.facility && `Facility: ${filters.facility}`, filters.reportingPeriod && `Period: ${filters.reportingPeriod}`, filters.dateFrom && `From: ${filters.dateFrom}`, filters.dateTo && `To: ${filters.dateTo}`]
            .filter(Boolean)
            .join(" · ")}
        </div>
      )}
